...
export const Success: React.FC<SuccessProps> = ({ userData, onReset }) => {
  ...
  const handleDownload = async () => {
    ...
    if (blob) {
      ...
      link.href = url;
      link.download = `Sparrow_Guardian_${userData.fullName.replace(/\s+/g, '_')}.png`;
  ...
  const handleShare = async () => {
    ...
    const blob = await generateImageBlob();
    if (blob) {
      const file = new File([blob], 'sparrow_guardian.png', { type: 'image/png' });
      if (navigator.share) {
        try {
          await navigator.share({
            files: [file],
            title: 'Sparrow Guardian Certificate',
            text: `🐦 I scored ${userData.score}/20 on the Sparrow Conservation Quiz! Join the movement. #SparrowGuardians`,
          });
        } catch (err) {
    ...
    <div className="min-h-screen bg-[#F2F0E9] pt-24 pb-16 px-4 flex flex-col items-center">
      <div className="w-full max-w-lg flex flex-col items-center text-center animate-fade-in">
        ...
        <p className="font-sans text-earth/60 mb-8">
          You scored <span className="font-bold text-leaf">{userData.score} / 20</span> and earned your Sparrow Guardian Certificate.
        </p>
